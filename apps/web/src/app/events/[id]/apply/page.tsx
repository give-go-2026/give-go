...
export default async function ApplyPage({
  ...
  const cardWithId = await fetchCardById(Number(id));
  if (!cardWithId) {
    return notFound();
  }
  ...
  return (
    <div className='flex min-h-screen flex-col items-center overflow-hidden'>
      <header className='main-gradient h-28 w-full px-3 py-6 text-center'>
        <h1 className='text-3xl font-black tracking-wide md:hidden'>
          Választott programod
        </h1>
      </header>
      <main className='-mt-6 w-full px-3'>
        <div className='mx-auto grid h-[430px] w-full max-w-[1030px] grid-cols-1 rounded-3xl bg-white shadow-2xl md:grid-cols-5'>
          <div className='col-span-full flex h-full w-full flex-col items-center justify-center p-0 md:col-span-2 md:rounded-l-3xl md:p-6'>
            <div className='h-full w-full overflow-clip rounded-3xl border border-slate-200 shadow-lg'>
              ...
              <div className='flex h-1/2 w-full flex-col gap-2 p-3'>
                <h4 className='text-xl tracking-wide text-pretty'>
                  {cardWithId.title}
                </h4>
                <span className='font-medium text-gray-500'>
                  {cardWithId.address}
                </span>
                <span className='flex grow items-center text-wrap'>
                  {cardWithId.description}
                </span>
              </div>
            </div>
          </div>
          <div className='hidden h-full w-full flex-col items-center justify-center p-6 text-center md:col-span-3 md:flex md:rounded-r-3xl'>
            <h1 className='mb-12 text-3xl font-black tracking-wide'>
              Választott programod
            </h1>
            <p className='mb-6 text-lg text-pretty'>
              Ahhoz hogy jelentkezz a programra, nyomd meg a gombot és küld el
              az emailt a céged kapcsolattartójának.
            </p>
            <div className='flex w-full gap-6'>
              <Link
                href={`mailto:${cardWithId.organizer_email}`}
                target='_blank'
                className='w-full'
              >
                <ApplyButton
      ...
      <footer className='mt-6 w-full px-3 text-center text-pretty md:mt-12'>
        ...
        <div className='md:hidden'>
          <p className='mb-3 text-lg text-pretty'>
            Ahhoz hogy jelentkezz a programra, nyomd meg a gombot és küld el az
            emailt a céged kapcsolattartójának.
          </p>
          <div className='mx-auto flex w-full flex-col gap-x-6 gap-y-3'>
            <Link
              href={`mailto:${cardWithId.organizer_email}?subject=Jelentkezés%20önkéntes%20programra:%20${cardWithId.title}`}
              target='_blank'
              rel='noreferrer'
            >
              <ApplyButton
